import { callMcpTool, toolText, toolItems, parseTable, type McpToolResult } from '../lib/mcp.js'
...
// Called on a timer by poller.ts — the sole source of Kubernetes polling now
// (no per-request fetching); server.ts just serves the latest cached result.
export async function collectKubernetes() {
  ...
  try {
    ...
    return { ok: false as const, error: e instanceof Error ? e.message : 'Kubernetes MCP server unreachable' }
  }
  ...
  await pushMetric('k8s-nodes', metricValues)
  const nodesHistory: MetricPoint[] = await getMetricHistory('k8s-nodes')
