...
import { callMcpTool, toolText, type McpToolResult } from '@/lib/mcp'
...
export async function GET() {
  ...
  if (
    ...
    return NextResponse.json({ ok: false, error: 'Home Assistant MCP server unreachable' })
  }
  ...
  const hostMetricsHistory: MetricPoint[] = getMetricHistory('ha-host')

  return NextResponse.json({ ok: true, integrations, apps, system, backups, hacs, hostMetricsHistory })
}
